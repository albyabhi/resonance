...
export default function ParticipantSetupPasswordPage() {
  ...
  useEffect(() => {
    ...
    apiJson(`${API()}/api/auth/validate-participant-setup/${token}`)
      .then((data) => {
        if (data.valid) {
          setValid(true);
          setParticipantInfo({ name: data.name, email: data.email });
        } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const data = await apiJson(`${API()}/api/auth/participant-setup-password`, {
        method: 'POST',
      ...
      <div className="min-h-screen bg-background flex flex-col items-center justify-center p-6 text-foreground">
        <div className="max-w-md w-full bg-card border border-border rounded-3xl p-8 shadow-xl relative overflow-hidden">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-sm text-muted-foreground mt-2">{error || 'This setup link has expired or has already been used.'}</p>
          </div>
        ...
        <div className="text-center mb-8">
          ...
          {participantInfo && (
            <p className="text-sm text-muted-foreground mt-2">
              Welcome, <strong>{participantInfo.name}</strong> ({participantInfo.email})
            </p>
          )}
        </div>
